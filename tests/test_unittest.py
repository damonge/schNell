import healpy as hp
import numpy as np
from snell import MapCalculator, GroundDetector, LISADetector
...
def test_antenna_new():
    ...
    aLL = np.abs(mcLL_n.get_antenna(0, 0, 0, 1E-2, theta, phi,
                                  inc_baseline=False))
    a11_test, a12_test, aLL_test = hp.read_map("tests/test_data/antenna_test.fits",
                                               field=None)
...
def test_Gell_new():
    ...
    ls, gl11_test, gl12_test, glLL_test = np.loadtxt("tests/test_data/gls_test.txt",
                                                     unpack=True)
